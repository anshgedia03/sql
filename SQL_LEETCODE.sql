...
SELECT department_name , 
        employee_name , 
        salary , 
        DENSE_RANK() OVER(PARTITION BY department_name ORDER BY salary DESC) as DEPT_WISE_RANK
FROM (SELECT 
            e.id, 
            e.name AS employee_name, 
            e.salary as salary, 
...
FROM (SELECT department_name , 
        ...
        DENSE_RANK() OVER(PARTITION BY department_name ORDER BY salary DESC) as DEPT_WISE_RANK
            ...
            d.name AS department_name
        FROM Employee e
        INNER JOIN Department d ON e.departmentId = d.id) as temp) as temp
WHERE temp.DEPT_WISE_RANK <= 3;
...
FROM (SELECT 
        ...
        employee_name as Employee,
        salary FROM answer WHERE DEPT_WISE_RANK <= 3 ;