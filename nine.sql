...
-- one way
LOAD DATA LOCAL INFILE 'C:/Users/ANSH/Desktop/Sales_Dataset.csv'
...
-- 3 rd highest state in term of shipping cost
SELECT state , MAX(shipping_cost) as ship_cost FROM sales_data GROUP BY state

--
SELECT * , DENSE_RANK() OVER(ORDER BY ship_cost DESC) as rank_state 
FROM (SELECT state , MAX(shipping_cost) as ship_cost FROM sales_data GROUP BY state) as temp


SELECT * 
FROM (SELECT * , DENSE_RANK() OVER(ORDER BY ship_cost DESC) as rank_state 
FROM (SELECT state , MAX(shipping_cost) as ship_cost FROM sales_data GROUP BY state) as temp
) as final_temp
WHERE rank_state = 3;
...
WITH states as(
 ...
 WITH answer as(
  SELECT * , DENSE_RANK() OVER(ORDER BY ship_cost DESC) as rank_state 
  FROM (SELECT state , MAX(shipping_cost) as ship_cost FROM sales_data GROUP BY state) as temp
) SELECT * from answer WHERE rank_state = 3 ;