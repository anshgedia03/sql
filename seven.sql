...
SELECT selectedCourses , COUNT(*) as entolled_in from learners GROUP BY selectedCourses



SELECT * , DENSE_RANK() OVER(ORDER BY entolled_in DESC) AS rank_of_enroll
    FROM (SELECT selectedCourses , COUNT(*) as entolled_in 
                    from learners GROUP BY selectedCourses) as temp
